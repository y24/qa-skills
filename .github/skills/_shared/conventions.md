# 共通規約(全スキル必読)

このファイルは `.github/skills/` 配下の全QAスキルが従う共通ルール。
各スキルは実行開始時にこのファイルを読み込むこと。

## 1. 言語と文体

- 出力・質問・成果物はすべて日本語。
- 成果物はMarkdown。構造化データ(分類結果・カタログ等)はYAMLブロックを併用する。

## 2. ユーザーとの対話ルール

ユーザーへの質問は、自由記述を求めず**選択肢から選ぶだけで完結する形**にする。
属人化を避け、誰が使っても一定水準の結果になるようにするため。

- 選択式質問ツール(`askQuestions`)が利用できる環境では、それを使う。
- 別名の同等ツール(例: `AskUserQuestion`)しかない環境では、それを使う。
- 質問ツールが一切ない環境では、番号付き選択肢をテキストで提示し、番号での回答を待つ。

質問の設計ルール:

- 1回の質問につき選択肢は2〜4個。常に「その他(自由記述)」相当の逃げ道を用意する。
- 複数選択が自然な質問(例: インプット資料の種類)は複数選択可にする。
- 推奨がある場合は先頭に置き「(推奨)」を付ける。

## 3. セッション管理(qa-session.json)

- 作業状態は作業ディレクトリ直下の `qa-output/<セッション名>/qa-session.json` に保存する。
- スキーマは [session-schema.md](session-schema.md) を参照。
- 各フェーズの開始時・完了時・承認時に必ず更新する。更新には
  `_shared/scripts/qa_session.py` を使う(§9。使えない環境では手動更新)。
- 中断からの再開時は、まず qa-session.json を読み、`plan` の未完了フェーズから続行する。

## 4. 承認ゲート

各フェーズの完了時に、成果物の要約を提示した上で必ずユーザーの承認を取る。
選択肢は次の4つ:

1. **承認して次へ(推奨)**
2. **修正を指示する** — 指示を受けて成果物を修正し、再度承認を求める
3. **このフェーズをやり直す**
4. **ここで中断する** — セッションを保存して終了(再開方法を案内する)

承認されるまで次のフェーズへ進んではならない。成果物は「AIが作ったもの」ではなく
「人間が承認したもの」として次フェーズに渡る。

## 5. 証拠レベル(推測と事実の分離)

分析・レビュー系の出力では、各指摘・各結論に証拠レベルを必ず付ける。

```yaml
evidence_level: confirmed   # 資料・コード・チケットで確認済み
evidence_level: likely      # 状況証拠あり。要確認
evidence_level: hypothesis  # 推測。裏取りが必要
sources:                    # 根拠(チケットID、ファイルパス、仕様書の節など)
  - BUG-1234
  - spec.md#3.2
```

根拠を示せない指摘は hypothesis とし、confirmed と混ぜて並べない。
「もっともらしいが間違っている分析」が最も有害であることを常に意識する。

## 6. 成果物の出力先と命名

```
qa-output/<セッション名>/
  qa-session.json
  sources/                  # Markdown変換済みインプット資料(§10。再生成可能な中間ファイル)
  00-code-overview.md
  01-defect-analysis.md
  02-test-analysis.md
  03-criteria-analysis.md
  04-spec-review-mode1.md
  05-test-planning.md
  06-feature-investigation.md
  07-code-review.md
  08-test-viewpoint.md
  09-spec-review-mode2.md
  10-test-case.md
  11-test-data.md
  12-test-design-review.md
  99-improvement.md
```

- 番号はフェーズ実行順。スキップされたフェーズは欠番でよい。
- `00-code-overview.md` はフェーズ外の事前把握資料(qa-code-overview)。セッション開始前に単独作成されることもある。
- 前フェーズの成果物は、次フェーズ開始時に必ず読み込む(バケツリレー)。

## 7. 不明点の扱い

- インプット資料に無い情報を勝手に補完しない。「不足情報」として成果物に明記する。
- 仕様の曖昧さを発見したら、その場で解釈を決めず `qa-spec-review` の検出フォーマットで記録する。
- 外部ナレッジベースが設定されているプロジェクトでは、不足情報の調査に
  [knowledge-base.md](knowledge-base.md) の参照プロトコルを使ってよい(任意。未設定なら無視する)。

## 8. 育てる資産の還元経路

`_shared/references/` のうち★印のファイル(セッションの知見を還元して育てる資産)は、
ファイルの性格によって還元経路が2系統に分かれる。

- **プロジェクト資産**(`regression-viewpoint-catalog.md` / `domain-glossary.md`):
  BUG-ID・用語などプロジェクト固有の知識。配布先リポジトリのセッション内で
  承認を経て直接追記して育てる。マスターへは反映しない。
- **マスター資産**(`review-checklist.md` / `code-review-viewpoints.md` /
  `defect-taxonomy.md` / 各 `SKILL.md` / `_shared/scripts/` のスクリプト / 本ファイル): プロジェクトを跨いで
  使える知見。セッション内では書き換えず、qa-improvement が振り返りレポート
  (99-improvement.md)に提案として一覧化し、メンテナーがマスターのリポジトリ上で
  `qa-skillset-maintenance` を使って採否を検討・反映する(採否履歴は
  [maintenance-log.md](maintenance-log.md) に蓄積)。

## 9. 補助スクリプト(定型はスクリプト、判断はAI)

定型的な処理 — セッションファイルの更新、ID突合、件数集計、組み合わせ生成、
書式チェック — はLLMが手作業で行わず、[_shared/scripts/](scripts/) のスクリプトに
委譲する。判断(ラベル付け・解釈・検出への対応方針)は従来どおりAIと人間の責務。

| スクリプト | 用途 | 主な利用箇所 |
|---|---|---|
| `qa_session.py` | qa-session.json の作成・更新・再開判定 | qa-orchestrator(全フェーズ境界) |
| `defect_stats.py` | 不具合CSVの正規化雛形生成とラベル集計 | qa-defect-analysis |
| `pairwise.py` | ペアワイズ組み合わせ生成(全ペア網羅を自己検証) | qa-test-case-design |
| `trace_check.py` | 成果物間のID突合(観点⇄ケース⇄QC⇄AMB) | qa-test-viewpoint / qa-test-case-design / qa-test-design-review |
| `lint_output.py` | 成果物の必須セクション・evidence_level・ID書式チェック | 全スキル(承認ゲート前)/ qa-improvement |

運用ルール:

- 実行には Python 3.9+ が必要(標準ライブラリのみ。追加インストール不要)。
  Python が使えない環境では従来どおり手動で行う — スクリプトは補助であり、
  ワークフロー自体は変わらない。
- 成果物は、承認ゲートで提示する前に `lint_output.py` を通し、ERROR を解消して
  から提示することが望ましい(WARN の扱いは判断に委ねる)。
- スクリプトの出力は機械処理の結果にすぎない。検出への対応要否・集計結果の
  意味づけはAIと人間が判断し、成果物にはその判断を書く。
- スクリプトはマスター資産(§8)。配布先セッション内では変更せず、改善は
  振り返りレポート経由でメンテナーに提案する。

## 10. インプット資料のMarkdown変換

インプット資料に Excel・PDF・Word・PowerPoint 等の直接読みにくい形式が
**含まれる場合のみ**、[source-conversion.md](source-conversion.md) を読み、
markitdown による Markdown 変換の手順に従う。
すべて Markdown・テキストの場合は読む必要はない。
