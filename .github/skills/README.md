...
| ツール | 方法 |
|---|---|
| GitHub Copilot | **専用のカスタムエージェント層 [`.github/agents/`](../agents/README.md) を同梱**(VS Code v1.107+)。本ディレクトリ `.github/skills/` は Copilot の Agent Skills 公式配置でもあるため、各スキルは Copilot から直接発見される。qa-orchestrator が `#tool:agent/runSubagent` で各フェーズエージェントを呼び出す。セットアップと入出力契約は同ディレクトリの README / qa-protocol.md を参照。質問ツールは `vscode/askQuestions` が使われる |
