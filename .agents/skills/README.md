...
.agents/skills/
  ...
  qa-spec-review/           # Phase 3/8: 仕様曖昧性検出(モード1=仕様、モード2=期待結果)
  qa-test-planning/         # Phase 4: テスト計画
  qa-feature-investigation/ # Phase 5: コード・デザインからの仕様補完
  qa-code-review/           # Phase 6: 品質特性ベースのQAコードレビュー(シフトレフト)
  qa-test-viewpoint/        # Phase 7: テスト観点抽出
  qa-test-case-design/      # Phase 9: テストケース展開
  qa-test-data-design/      # Phase 10: テストデータ設計
  qa-test-design-review/    # Phase 11: 独立レビュー(S〜D評価)
  qa-improvement/           # Phase 99: スキルセット自己進化
  _shared/
    conventions.md          # 全スキル共通規約(対話・承認ゲート・証拠レベル)
    session-schema.md       # qa-session.json のスキーマ
    knowledge-source.md     # ★LLM-wiki連携(ドメイン知識の参照プロトコル)
    references/             # AIが読むナレッジ(育てる資産)
      ...
      domain-glossary.md                # wiki未設定時のフォールバック+ingestインボックス
```
...
6. **育てる資産** — ★印のファイルはセッションごとに qa-improvement が追記し、
   スキルセットが自分の現場に適応していく。
7. **ドメイン知識は wiki に委譲** — ドメイン用語・業務ルールの正は、スキルセットの外に
   ある LLM-wiki(memo/llm-wiki.md のパターンで構築したもの)。スキルは
   `_shared/knowledge-source.md` のプロトコル(index ファースト → 必要ページのみ →
   全文検索)で**読み取り専用**参照し、新知識は ingest 候補として wiki 側の
   フローに還元する。QAナレッジ(回帰観点・レビュー観点)は references/ に、
   ドメインナレッジは wiki に、と置き場所を分けるのが原則。
