...
- [_shared/gates.md](../_shared/gates.md)(**ゲートの実施手順** — レビュー依頼・承認の4択・止まり方)
...
- **存在し、未完了ステップがある場合**: セッション概要(対象・実行モード・承認済みゲート・次のステップ)を提示し、「続きから再開 / 新規セッション開始」を選択式で確認する
  - **`awaiting_approval` のゲートがある場合**は、そこで中断したということ。再開時にまず**そのゲートのレビュー依頼を出し直す**(gates.md §4-1)。「G<N> が未承認です」とだけ報告して止まらない
- **存在しない場合**: Step 1 へ
...
3. 任意で挿入できるスキルを判定する(不具合一覧があれば `qa-defect-analysis`)
4. 各ステップに §3 のゲートを割り当てる
5. **計画案を提示**する: 実行するスキル・スキップするスキルとその理由・ゲートの位置・各ゲートで誰の承認が要るか

これが **G1(スコープ)ゲート**。gates.md §4-1 のレビュー依頼の形(特に**スキップするスキルとその理由**・**選んだモードの限界**)で提示し、gates.md §4-2 の4択で承認を得てからセッションを作成する。
...
3. **要約(全文ではない)+ 重要な判断ポイント + 残った不明点 + ゲートで見てほしい点(1〜3点)**を提示する。ここでは承認を取らず、異議がなければ次へ進む(見てほしい点はゲートのレビュー依頼の材料になる — gates.md §4-1)
...
1. `gate_check.py <dir> --gate <ゲート>` で機械検証を通す(残る検出は解消する。解消できないものは gates.md §4-3 に従い理由を添えて提示する)
2. **レビュー依頼を出す**(gates.md §4-1)。開くファイルのパス・見てほしい箇所3〜7点(各点に「AIの現在の判断」と「なぜ確認が要るか」)・確認不要な範囲(機械検証済み)を書く。ゲートごとに必ず指す箇所は skill-map.md §3-1
3. 同じ質問の中で gates.md §4-2 の4択を出し、**その回答で承認・差し戻しを完結させる**(確認の往復を挟まない)。出した時点で `qa_session.py set-gate <dir> <ゲート> awaiting_approval --note "<何を見てもらっているか>"` を記録する
...
**未承認のまま止まらない。** 「G<N> が未承認なので進めません」と書きたくなったら、それはレビュー依頼(手順2)をまだ出していないということ。ユーザーが「進めて」「承認」と言った場合はそれが承認であり、何を承認したことになるかを1〜3行で示して `set-gate <ゲート> approved --note "内容未確認のまま承認"` で記録し、進む(gates.md §4-3)。
