...
`.github/agents/` 配下の QA エージェント群が `#tool:agent/runSubagent` で呼び出されるときの共通契約。各エージェントの**手順の本体**は `.github/skills/<エージェント名>/SKILL.md` にあり、このファイルは「サブエージェントとして呼ばれたときの入出力と制約」だけを定める。
...
1. **ユーザーへ質問できない**(askQuestions 等は親しか使えない)。SKILL.md の手順に「ユーザーに確認する」「承認を得る」とある箇所は、その場で止めず `pending_questions` / `proposals` に変換して親へ返す。
...
入力に `"mode": "subagent"` の JSON が**含まれない**場合(ユーザーがチャットから直接エージェントを選んだ場合)は、このプロトコルは適用しない。SKILL.md の手順どおり、選択式質問・承認を挟みながら対話的に実行する。
