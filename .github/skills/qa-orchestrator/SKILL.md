...
description: QA業務全体の入口。テスト対象のヒアリングから実行計画の立案、各QAスキル(不具合分析→テスト分析→品質基準策定→仕様レビュー→テスト計画→機能調査→コードレビュー→テスト観点→テストケース→テストデータ→設計レビュー)の順次実行と承認ゲート管理、セッションの中断・再開までを統括する。「QAを始めたい」「テスト設計をしたい」「前回の続きから」と言われたら使う。
---
