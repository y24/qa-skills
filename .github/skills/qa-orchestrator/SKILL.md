...
ここで聞くのは**人にしか答えられないこと**(資料の所在・モード・範囲・重点・除外)だけ。実装の詳細は必要になった時点で §2-1 の選択式にする。
...
1. 該当スキルを実行する。前段の成果物を必ずインプットに含める(**バケツリレー**)。読むべき成果物は skill-map.md §1 の入力欄が定める
   - **コンテキスト分離する場合**: [subagent-contract.md](../_shared/subagent-contract.md) の入力JSONで起動し、返ってきた出力JSONの `pending_questions` / `proposals` を親であるこのスキルが処理する。実装の挙動を問う質問も、選択肢の先頭に「設計書・実装を確認して確定する(推奨)」があることを確かめてユーザーに出す(conventions.md §2-1)。調査は許可を得た子スキルにさせる
...
- ヒアリングを自由記述の質問で行うこと。
- **実装の挙動・詳細仕様を「確認して確定する」の選択肢なしに扱うこと**(conventions.md §2-1)。逆に**判断を仰ぐ質問を自分で決めて済ませること**も違反。
