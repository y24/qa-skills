...
.github/skills/
  ...
  _shared/
    conventions.md          # 全スキル共通規約(対話・承認ゲート・証拠レベル・資料のMarkdown変換)
    session-schema.md       # qa-session.json のスキーマ
    knowledge-base.md       # 外部ナレッジベース(OKF×LLM Wiki)連携の設定と参照プロトコル(任意)
    source-conversion.md    # Excel/PDF等→Markdown変換(markitdown)の手順(変換対象があるときのみ読む)
    diff-acquisition.md     # PR・コード差分の取得手順(対象がPR・diffのときのみ読む)
    ...
    references/             # AIが読むナレッジ(育てる資産)
      ...
      defect-taxonomy.md
      regression-viewpoint-catalog.md   # ★過去不具合→回帰観点の蓄積場所(発生メカニズム節を含む)
...
- markitdown が無い環境では従来どおり原本を直接読む(ワークフローは変わらない)
...
3. 実際のテスト設計書に対して `qa-test-design-review` を実行 → カタログの観点と
   発生メカニズムの机上シミュレーション(この壊れ方を検出できる設計か)が
   レビューに効くことを確認
