...
チェック項目:
    ...
    5. AMB参照の実在確認  : AMB-NNN 参照が仕様レビューのいずれかに定義されているか
        ...
        for fname, text in list(vp_texts.items()) + list(tc_texts.items()):
            for amb_id in _AMB_RE.findall(text):
                ...
                findings.append({
                    ...
                    "detail": f"{fname}: {amb_id}(どの仕様レビューファイルにも定義がない)",
                })
        note = None
        if not sr_files:
            if findings:
                note = "仕様レビューファイルが存在しないため、全AMB参照が未定義扱い"
            else:
                add_check(5, "AMB参照の実在確認", "skipped",
                          note="仕様レビューファイルなし・AMB参照もなし")
                findings = None
