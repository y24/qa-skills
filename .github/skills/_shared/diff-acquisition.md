...
変更行の**行レンジだけ**を安価に把握したいとき(どのファイルの何行目が変わったかの
地図作り)は、本文なしのハンク見出しを使う:

```
git diff --unified=0 origin/<target>...origin/<source> | grep -E '^@@|^diff --git'
```

この「ファイル → 変更行レンジ」の地図は、指摘が今回の変更由来か既存コード由来かの
区別(qa-code-review 手順6)にも使える。
...
- **先に深く読む**: 認証・権限、金額・数量の計算、バリデーションや
  チェック処理の削除・変更、多数の呼び出し元を持つ共通部品、DBマイグレーション
...
- 1グループ(大きければ1ファイル)ずつ取得する。差分だけで判断できない箇所は
  `git show origin/<source>:<パス>` でファイル全体を読む。呼び出し元・呼び出し先を
  横断検索したい場合は `git worktree add <一時ディレクトリ> origin/<source>` で
  別ディレクトリに展開する。**ユーザーの作業ツリーで checkout してブランチを
  切り替えない**(未コミットの変更や開いているエディタを壊す)。
...
- バリデーション・ガード・権限チェックなどの**削除・弱体化**を見つけたら、
  `git log -S "<削除されたコード>" --oneline` でその行の由来を確認する。
  過去の不具合修正(fix・BUG ID を含むコミット)で入ったコードが消えていたら、
  回帰リスクとして扱う(qa-code-review では回帰観点カタログとの突合材料になる)。
