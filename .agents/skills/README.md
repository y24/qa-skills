...
6. **育てる資産** — ★印のファイルにセッションの知見を還元して育てる。qa-improvement が
   追記候補を振り返りレポート(99-improvement.md)に一覧化する。
   レポートはメンテナーがマスターのリポジトリ上で `qa-skillset-maintenance` を使って
   採否を検討し、採用分を反映する(採否履歴は `_shared/maintenance-log.md` に蓄積)。

## 外部ナレッジベースとの連携(任意)

ドメイン知識が多く用語集1ファイルに収まらないプロジェクトでは、OKF × LLM Wiki 形式で
構築したQAナレッジベース(別リポジトリ。`knowledge/{features,glossary,rules,decisions}` +
kb-ingest/kb-query/kb-lint)を**追加のドメイン知識ソース**として参照できる。

- 設定・参照プロトコル・証拠レベルの扱いは [_shared/knowledge-base.md](_shared/knowledge-base.md) に集約。
- **必須前提ではない**。KB がないプロジェクトは domain-glossary.md だけでこれまで通り動く。
- KB は読み取り専用。セッションで得た新知識は domain-glossary.md の「KB ingest 待ち」節に
  溜め、ユーザーが KB 側の kb-ingest で取り込む(一方向の還元)。
